...
openai.api_key = "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"
...
bot = telebot.TeleBot("5820608633:AAEM9ZPxDKRO6T-XtweeEA4J-rRgCa7fMaY")
    ...
    try:
        if message.chat.type == "private":
            ...
            bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                             text="Use this command in a group and reply to a message.")
            return
        ...
        if len(response["choices"][0]["text"]) > 3900:
            bot.send_message(chat_id=message.chat.id, reply_to_message_id=message.message_id,
                             text="Message is too big, Telegram doesn't support")
        else:
...
def generate_gpt(message):
    try:
        ...
        if len(prompt_array) < 2:
            bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                             text="Please provide a message after the command '/gpt'")
            return
...
def generate_trengpt(message):
    try:
        ...
        if len(prompt_array) < 1:
            bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                             text="Please provide a message after the command '/tl' or reply to a Message")
            return
...
def generate_gptaudio(message):
    try:
        ...
        if len(prompt_array) < 2:
            bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                             text="Please provide a message after the command '/gpta'")
            return
...
def generate_dan(message):
    try:
        ...
        if len(prompt_array) < 2:
            bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                             text="Please provide a message after the command '/dan'")
            return
...
def generate_bug(message):
    ...
    if len(prompt_array) < 1:
        bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                         text="Please provide a message after the command '/bug'")
        return
    ...
    try:
        ...
        bot.send_message(chat_id=message.chat.id, reply_to_message_id=message.message_id,
                         text=f"An error occurred while calling OpenAI API: {e}")
        return
    ...
    if len(response_text) > 3900:
        bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                         text="The response message is too long for Telegram (maximum 3900 characters). Please try again with a shorter message.")
        return
...
def generate_karen(message):
    ...
    if len(prompt_array) < 1:
        bot.send_message(chat_id=chat_id, reply_to_message_id=message.message_id,
                         text="Please provide a message after the command '/karen'")
        return
